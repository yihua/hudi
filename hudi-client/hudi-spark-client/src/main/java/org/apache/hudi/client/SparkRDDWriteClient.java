...
public class SparkRDDWriteClient<T extends HoodieRecordPayload> extends
  ...
  protected void completeCompaction(HoodieCommitMetadata metadata,
    ...
    try {
      if (!basePath.contains(".hoodie/metadata")) {
        killJVMIfDesired("/tmp/fail82_dt_compaction.txt", "Fail data table compaction before applying to MDT " + compactionCommitTime, 0.2);
      }
      ...
      if (basePath.contains(".hoodie/metadata")) {
        killJVMIfDesired("/tmp/fail82_mt_compaction.txt", "Fail metadata table compaction " + compactionCommitTime, 0.2);
      } else {
  ...
  private void completeClustering(HoodieReplaceCommitMetadata metadata,
    ...
    try {
      if (!basePath.contains(".hoodie/metadata")) {
        killJVMIfDesired("/tmp/fail81_dt_clustering.txt", "Fail data table clustering before applying to MDT " + clusteringCommitTime, 0.2);
      }
      ...
      if (basePath.contains(".hoodie/metadata")) {
        killJVMIfDesired("/tmp/fail81_mt_clustering.txt", "Fail metadata table clustering " + clusteringCommitTime, 0.2);
      } else {
