  ...
  protected void completeCompaction(HoodieCommitMetadata metadata,
    ...
    try {
      ...
      if (basePath.contains(".hoodie/metadata")) {
        killJVMIfDesired("/tmp/fail32_mt_compaction.txt", "Fail metadata table compaction", 3);
      } else {
        killJVMIfDesired("/tmp/fail62_dt_compaction.txt", "Fail data table compaction", 3);
      }
