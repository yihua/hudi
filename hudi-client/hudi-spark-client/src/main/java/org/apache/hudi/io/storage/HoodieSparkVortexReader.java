...
/**
 * {@link HoodieSparkFileReader} implementation for Vortex file format.
 */
@Slf4j
public class HoodieSparkVortexReader implements HoodieSparkFileReader {
  ...
  public HoodieSparkVortexReader(StoragePath path, long dataAllocatorSize, long metadataAllocatorSize) {
    this.path = path;
    this.dataAllocatorSize = dataAllocatorSize;
    try {
      metadataSession = Session.create();
      metadataDataSource = metadataSession.openDataSource(path.toString());
      arrowSchema = metadataDataSource.schema();
    } catch (Exception e) {
      close();
  ...
  public String[] readMinMaxRecordKeys() {
    ...
    throw new HoodieException("Could not read min/max record key out of Vortex file: " + path);
  }
  ...
  private ClosableIterator<UnsafeRow> getUnsafeRowIterator(HoodieSchema requestedSchema) {
    ...
    try {
      ...
      DataSource dataSource = session.openDataSource(path.toString());
      ...
      Scan scan = dataSource.newScan()
          .project(columnNames.toArray(new String[0]))
          .batchSize(DEFAULT_BATCH_SIZE)
          .build();
      ArrowReader arrowReader = scan.toArrowReader(allocator);

      return new VortexRecordIterator(allocator, dataSource, session, arrowReader, requestedSparkSchema, path.toString());
    } catch (Exception e) {
      allocator.close();
  ...
  public void close() {
    if (metadataDataSource != null) {
      try {
        ...
        log.warn("Error while closing Vortex metadata data source: {}", e.getMessage());
      }
    }
    if (metadataSession != null) {
      try {
        ...
        log.warn("Error while closing Vortex metadata session: {}", e.getMessage());
      }
    }
  }
  ...
  public long getTotalRecords() {
    try {
      return metadataDataSource.numRows();
    } catch (Exception e) {
