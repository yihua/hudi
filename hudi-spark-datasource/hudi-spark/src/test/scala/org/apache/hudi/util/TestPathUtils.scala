...
class TestPathUtils {
  ...
  def testGlobPaths(@TempDir tempDir: File): Unit = {
    val folders: Seq[HoodieLocation] = Seq(
      ...
      new HoodieLocation(Paths.get(tempDir.getAbsolutePath, ".hoodie").toUri),
      new HoodieLocation(Paths.get(tempDir.getAbsolutePath, ".hoodie", "metadata").toUri)
    )

    val files: Seq[HoodieLocation] = Seq(
      new HoodieLocation(Paths.get(tempDir.getAbsolutePath, "folder1", "file1").toUri),
      new HoodieLocation(Paths.get(tempDir.getAbsolutePath, "folder1", "file2").toUri),
      new HoodieLocation(Paths.get(tempDir.getAbsolutePath, "folder2", "file3").toUri),
      new HoodieLocation(Paths.get(tempDir.getAbsolutePath, "folder2", "file4").toUri),
      new HoodieLocation(Paths.get(tempDir.getAbsolutePath, ".hoodie", "metadata", "file5").toUri),
      new HoodieLocation(Paths.get(tempDir.getAbsolutePath, ".hoodie", "metadata", "file6").toUri)
    )
