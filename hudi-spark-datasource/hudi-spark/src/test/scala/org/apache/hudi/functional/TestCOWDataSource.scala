...
class TestCOWDataSource extends HoodieSparkClientTestBase with ScalaAssertionSupport {
  ...
  def testInferPartitionBy(): Unit = {
    ...
    val storage = HoodieStorageUtils.getHoodieStorage(new HoodieLocation(basePath), new Configuration())
    assertTrue(storage.exists(new HoodieLocation(basePath + "/" + HoodieTestDataGenerator.DEFAULT_FIRST_PARTITION_PATH)))
    assertTrue(storage.exists(new HoodieLocation(basePath + "/" + HoodieTestDataGenerator.DEFAULT_SECOND_PARTITION_PATH)))
    assertTrue(storage.exists(new HoodieLocation(basePath + "/" + HoodieTestDataGenerator.DEFAULT_THIRD_PARTITION_PATH)))
