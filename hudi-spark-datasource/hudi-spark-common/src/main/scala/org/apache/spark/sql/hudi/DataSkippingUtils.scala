...
object DataSkippingUtils extends Logging {
   ...
   */
  def translateIntoColumnStatsIndexFilterExpr(dataTableFilterExpr: Expression, indexSchema: StructType): (String, Expression) = {
    try {
  ...
  private def createColumnStatsIndexFilterExprInternal(dataTableFilterExpr: Expression, indexSchema: StructType): (String, Expression) = {
  ...
  private def tryComposeIndexFilterExpr(sourceExpr: Expression, indexSchema: StructType): Option[(String, Expression)] = {
    //
    ...
    sourceExpr match {
      ...
      case EqualTo(sourceExpr @ AllowedTransformationExpression(attrRef), valueExpr: Expression) if isValueExpression(valueExpr) =>
        getTargetIndexedColumnName(attrRef, indexSchema)
          .map { colName =>
            ...
            (colName, genColumnValuesEqualToExpression(colName, valueExpr, targetExprBuilder))
          }
      ...
      case Not(EqualTo(sourceExpr @ AllowedTransformationExpression(attrRef), value: Expression)) if isValueExpression(value) =>
        getTargetIndexedColumnName(attrRef, indexSchema)
          .map { colName =>
            val targetExprBuilder: Expression => Expression = swapAttributeRefInExpr(sourceExpr, attrRef, _)
            (colName, Not(genColumnOnlyValuesEqualToExpression(colName, value, targetExprBuilder)))
          }
      ...
      case EqualNullSafe(attrRef: AttributeReference, litNull @ Literal(null, _)) =>
        getTargetIndexedColumnName(attrRef, indexSchema)
          .map(colName => (colName, EqualTo(genColNumNullsExpr(colName), litNull)))
      ...
      case LessThan(sourceExpr @ AllowedTransformationExpression(attrRef), value: Expression) if isValueExpression(value) =>
        getTargetIndexedColumnName(attrRef, indexSchema)
          .map { colName =>
            val targetExprBuilder: Expression => Expression = swapAttributeRefInExpr(sourceExpr, attrRef, _)
            (colName, LessThan(targetExprBuilder.apply(genColMinValueExpr(colName)), value))
          }
      ...
      case LessThan(value: Expression, sourceExpr @ AllowedTransformationExpression(attrRef)) if isValueExpression(value) =>
        getTargetIndexedColumnName(attrRef, indexSchema)
          .map { colName =>
            val targetExprBuilder: Expression => Expression = swapAttributeRefInExpr(sourceExpr, attrRef, _)
            (colName, GreaterThan(targetExprBuilder.apply(genColMaxValueExpr(colName)), value))
          }
      ...
      case LessThanOrEqual(sourceExpr @ AllowedTransformationExpression(attrRef), value: Expression) if isValueExpression(value) =>
        getTargetIndexedColumnName(attrRef, indexSchema)
          .map { colName =>
            val targetExprBuilder: Expression => Expression = swapAttributeRefInExpr(sourceExpr, attrRef, _)
            (colName, LessThanOrEqual(targetExprBuilder.apply(genColMinValueExpr(colName)), value))
          }
      ...
      case LessThanOrEqual(value: Expression, sourceExpr @ AllowedTransformationExpression(attrRef)) if isValueExpression(value) =>
        getTargetIndexedColumnName(attrRef, indexSchema)
          .map { colName =>
            val targetExprBuilder: Expression => Expression = swapAttributeRefInExpr(sourceExpr, attrRef, _)
            (colName, GreaterThanOrEqual(targetExprBuilder.apply(genColMaxValueExpr(colName)), value))
          }
      ...
      case IsNull(attribute: AttributeReference) =>
        getTargetIndexedColumnName(attribute, indexSchema)
          .map(colName => (colName, GreaterThan(genColNumNullsExpr(colName), Literal(0))))

      // Filter "colA is not null"
      // Translates to "colA_nullCount < colA_valueCount" for index lookup
      case IsNotNull(attribute: AttributeReference) =>
        getTargetIndexedColumnName(attribute, indexSchema)
          .map(colName => (colName, LessThan(genColNumNullsExpr(colName), genColValueCountExpr)))
      ...
      case In(sourceExpr @ AllowedTransformationExpression(attrRef), list: Seq[Expression]) if list.forall(isValueExpression) =>
        getTargetIndexedColumnName(attrRef, indexSchema)
          .map { colName =>
            val targetExprBuilder: Expression => Expression = swapAttributeRefInExpr(sourceExpr, attrRef, _)
            (colName, list.map(lit => genColumnValuesEqualToExpression(colName, lit, targetExprBuilder)).reduce(Or))
          }
      ...
      case Not(In(sourceExpr @ AllowedTransformationExpression(attrRef), list: Seq[Expression])) if list.forall(_.foldable) =>
        getTargetIndexedColumnName(attrRef, indexSchema)
          .map { colName =>
            val targetExprBuilder: Expression => Expression = swapAttributeRefInExpr(sourceExpr, attrRef, _)
            (colName, Not(list.map(lit => genColumnOnlyValuesEqualToExpression(colName, lit, targetExprBuilder)).reduce(Or)))
          }
      ...
      case StartsWith(sourceExpr @ AllowedTransformationExpression(attrRef), v @ Literal(_: UTF8String, _)) =>
        getTargetIndexedColumnName(attrRef, indexSchema)
          .map { colName =>
            val targetExprBuilder: Expression => Expression = swapAttributeRefInExpr(sourceExpr, attrRef, _)
            (colName, genColumnValuesEqualToExpression(colName, v, targetExprBuilder))
          }
      ...
      case Not(StartsWith(sourceExpr @ AllowedTransformationExpression(attrRef), value @ Literal(_: UTF8String, _))) =>
        getTargetIndexedColumnName(attrRef, indexSchema)
          .map { colName =>
            ...
            (colName, Not(And(StartsWith(minValueExpr, value), StartsWith(maxValueExpr, value))))
          }
      ...
      case or: Or =>
        ...
        assert("".equals(resLeft._1) || "".equals(resRight._1) || resLeft._1.equals(resRight._1))
        Option((resLeft._1, Or(resLeft._2, resRight._2)))
      ...
      case and: And =>
        ...
        assert("".equals(resLeft._1) || "".equals(resRight._1) || resLeft._1.equals(resRight._1))
        Option((resLeft._1, And(resLeft._2, resRight._2)))
   ...
   * @return filter for column-stats index's table
   */
  def newTranslateIntoColumnStatsIndexFilterExpr(dataTableFilterExpr: Expression, indexSchema: StructType): (String, Expression) = {
    try {
      newCreateColumnStatsIndexFilterExprInternal(dataTableFilterExpr, indexSchema)
    } catch {
      case e: AnalysisException =>
        logDebug(s"Failed to translated provided data table filter expr into column stats one ($dataTableFilterExpr)", e)
        throw e
  ...
  private def newCreateColumnStatsIndexFilterExprInternal(dataTableFilterExpr: Expression, indexSchema: StructType): (String, Expression) = {
    // Try to transform original Source Table's filter expression into
    // Column-Stats Index filter expression
    newTryComposeIndexFilterExpr(dataTableFilterExpr, indexSchema) match {
      case Some(e) => e
      // NOTE: In case we can't transform source filter expression, we fallback
      // to {@code TrueLiteral}, to essentially avoid pruning any indexed files from scanning
      case None => ("", TrueLiteral)
  ...
  private def newTryComposeIndexFilterExpr(sourceExpr: Expression, indexSchema: StructType): Option[(String, Expression)] = {
    //
    // For translation of the Filter Expression for the Data Table into Filter Expression for Column Stats Index, we're
    // assuming that
    //    - The column A is queried in the Data Table (hereafter referred to as "colA")
    ...
    sourceExpr match {
      ...
      case EqualTo(sourceExpr@AllowedTransformationExpression(attrRef), valueExpr: Expression) if isValueExpression(valueExpr) =>
        getTargetIndexedColumnName(attrRef, indexSchema)
          .map { colName =>
            ...
            //       corresponding column in the Column Stats Index
            val targetExprBuilder: Expression => Expression = swapAttributeRefInExpr(sourceExpr, attrRef, _)
            (colName, newGenColumnValuesEqualToExpression(colName, valueExpr, targetExprBuilder))
          }
      ...
      case Not(EqualTo(sourceExpr@AllowedTransformationExpression(attrRef), value: Expression)) if isValueExpression(value) =>
        getTargetIndexedColumnName(attrRef, indexSchema)
          .map { colName =>
            val targetExprBuilder: Expression => Expression = swapAttributeRefInExpr(sourceExpr, attrRef, _)
            (colName, Not(newGenColumnOnlyValuesEqualToExpression(colName, value, targetExprBuilder)))
          }
      ...
      case EqualNullSafe(attrRef: AttributeReference, litNull@Literal(null, _)) =>
        getTargetIndexedColumnName(attrRef, indexSchema)
          .map(colName => (colName, EqualTo(col(HoodieMetadataPayload.COLUMN_STATS_FIELD_NULL_COUNT).expr, litNull)))
      ...
      case LessThan(sourceExpr@AllowedTransformationExpression(attrRef), value: Expression) if isValueExpression(value) =>
        getTargetIndexedColumnName(attrRef, indexSchema)
          .map { colName =>
            val targetExprBuilder: Expression => Expression = swapAttributeRefInExpr(sourceExpr, attrRef, _)
            (colName, LessThan(targetExprBuilder.apply(col(HoodieMetadataPayload.COLUMN_STATS_FIELD_MIN_VALUE).expr), value))
          }
      ...
      case LessThan(value: Expression, sourceExpr@AllowedTransformationExpression(attrRef)) if isValueExpression(value) =>
        getTargetIndexedColumnName(attrRef, indexSchema)
          .map { colName =>
            val targetExprBuilder: Expression => Expression = swapAttributeRefInExpr(sourceExpr, attrRef, _)
            (colName, GreaterThan(targetExprBuilder.apply(col(HoodieMetadataPayload.COLUMN_STATS_FIELD_MAX_VALUE).expr), value))
          }
      ...
      case LessThanOrEqual(sourceExpr@AllowedTransformationExpression(attrRef), value: Expression) if isValueExpression(value) =>
        getTargetIndexedColumnName(attrRef, indexSchema)
          .map { colName =>
            val targetExprBuilder: Expression => Expression = swapAttributeRefInExpr(sourceExpr, attrRef, _)
            (colName, LessThanOrEqual(targetExprBuilder.apply(col(HoodieMetadataPayload.COLUMN_STATS_FIELD_MIN_VALUE).expr), value))
          }
      ...
      case LessThanOrEqual(value: Expression, sourceExpr@AllowedTransformationExpression(attrRef)) if isValueExpression(value) =>
        getTargetIndexedColumnName(attrRef, indexSchema)
          .map { colName =>
            val targetExprBuilder: Expression => Expression = swapAttributeRefInExpr(sourceExpr, attrRef, _)
            (colName, GreaterThanOrEqual(targetExprBuilder.apply(col(HoodieMetadataPayload.COLUMN_STATS_FIELD_MAX_VALUE).expr), value))
          }
      ...
      case IsNull(attribute: AttributeReference) =>
        getTargetIndexedColumnName(attribute, indexSchema)
          .map(colName => (colName, GreaterThan(col(HoodieMetadataPayload.COLUMN_STATS_FIELD_NULL_COUNT).expr, Literal(0))))

      // Filter "colA is not null"
      // Translates to "colA_nullCount < colA_valueCount" for index lookup
      case IsNotNull(attribute: AttributeReference) =>
        getTargetIndexedColumnName(attribute, indexSchema)
          .map(colName => (colName, LessThan(col(HoodieMetadataPayload.COLUMN_STATS_FIELD_NULL_COUNT).expr, genColValueCountExpr)))
      ...
      case In(sourceExpr@AllowedTransformationExpression(attrRef), list: Seq[Expression]) if list.forall(isValueExpression) =>
        getTargetIndexedColumnName(attrRef, indexSchema)
          .map { colName =>
            val targetExprBuilder: Expression => Expression = swapAttributeRefInExpr(sourceExpr, attrRef, _)
            (colName, list.map(lit => newGenColumnValuesEqualToExpression(colName, lit, targetExprBuilder)).reduce(Or))
          }
      ...
      case Not(In(sourceExpr@AllowedTransformationExpression(attrRef), list: Seq[Expression])) if list.forall(_.foldable) =>
        getTargetIndexedColumnName(attrRef, indexSchema)
          .map { colName =>
            val targetExprBuilder: Expression => Expression = swapAttributeRefInExpr(sourceExpr, attrRef, _)
            (colName, Not(list.map(lit => newGenColumnOnlyValuesEqualToExpression(colName, lit, targetExprBuilder)).reduce(Or)))
          }
      ...
      case StartsWith(sourceExpr@AllowedTransformationExpression(attrRef), v@Literal(_: UTF8String, _)) =>
        getTargetIndexedColumnName(attrRef, indexSchema)
          .map { colName =>
            val targetExprBuilder: Expression => Expression = swapAttributeRefInExpr(sourceExpr, attrRef, _)
            (colName, newGenColumnValuesEqualToExpression(colName, v, targetExprBuilder))
          }
      ...
      case Not(StartsWith(sourceExpr@AllowedTransformationExpression(attrRef), value@Literal(_: UTF8String, _))) =>
        getTargetIndexedColumnName(attrRef, indexSchema)
          .map { colName =>
            val targetExprBuilder: Expression => Expression = swapAttributeRefInExpr(sourceExpr, attrRef, _)
            val minValueExpr = targetExprBuilder.apply(col(HoodieMetadataPayload.COLUMN_STATS_FIELD_MIN_VALUE).expr)
            val maxValueExpr = targetExprBuilder.apply(col(HoodieMetadataPayload.COLUMN_STATS_FIELD_MAX_VALUE).expr)
            (colName, Not(And(StartsWith(minValueExpr, value), StartsWith(maxValueExpr, value))))
          }

      case or: Or =>
        val resLeft = newCreateColumnStatsIndexFilterExprInternal(or.left, indexSchema)
        val resRight = newCreateColumnStatsIndexFilterExprInternal(or.right, indexSchema)

        assert("".equals(resLeft._1) || "".equals(resRight._1) || resLeft._1.equals(resRight._1))
        Option((resLeft._1, Or(resLeft._2, resRight._2)))

      case and: And =>
        val resLeft = newCreateColumnStatsIndexFilterExprInternal(and.left, indexSchema)
        val resRight = newCreateColumnStatsIndexFilterExprInternal(and.right, indexSchema)
      ...
      case Not(And(left: Expression, right: Expression)) =>
        Option(newCreateColumnStatsIndexFilterExprInternal(Or(Not(left), Not(right)), indexSchema))

      case Not(Or(left: Expression, right: Expression)) =>
        Option(newCreateColumnStatsIndexFilterExprInternal(And(Not(left), Not(right)), indexSchema))
...
private object ColumnStatsExpressionUtils {
  ...
  @inline def newGenColumnValuesEqualToExpression(colName: String,
                                                  value: Expression,
                                                  targetExprBuilder: Function[Expression, Expression] = Predef.identity): Expression = {
    val minValueExpr = targetExprBuilder.apply(col(HoodieMetadataPayload.COLUMN_STATS_FIELD_MIN_VALUE).expr)
    val maxValueExpr = targetExprBuilder.apply(col(HoodieMetadataPayload.COLUMN_STATS_FIELD_MAX_VALUE).expr)
  ...
  def newGenColumnOnlyValuesEqualToExpression(colName: String,
                                              value: Expression,
                                              targetExprBuilder: Function[Expression, Expression] = Predef.identity): Expression = {
    val minValueExpr = targetExprBuilder.apply(col(HoodieMetadataPayload.COLUMN_STATS_FIELD_MIN_VALUE).expr)
    val maxValueExpr = targetExprBuilder.apply(col(HoodieMetadataPayload.COLUMN_STATS_FIELD_MAX_VALUE).expr)
