...
import org.apache.spark.sql.internal.SQLConf.{HIVE_FILESOURCE_PARTITION_FILE_CACHE_SIZE, HIVE_MANAGE_FILESOURCE_PARTITIONS}
...
/**
 * Use [[SparkHoodieFileStatusCache.getOrCreate()]] to construct a globally shared file status cache.
 */
object SparkHoodieFileStatusCache {
   ...
   */
  def getOrCreate(session: SparkSession): SparkHoodieFileStatusCache = synchronized {
    val filesourcePartitionFileCacheSize = session.sqlContext.getConf(
      HIVE_FILESOURCE_PARTITION_FILE_CACHE_SIZE.key,
      HIVE_FILESOURCE_PARTITION_FILE_CACHE_SIZE.defaultValueString).toLong
    if (session.sqlContext.getConf(
      HIVE_MANAGE_FILESOURCE_PARTITIONS.key,
      HIVE_MANAGE_FILESOURCE_PARTITIONS.defaultValueString).toBoolean &&
      filesourcePartitionFileCacheSize > 0) {
      if (sharedCache == null) {
 ...
 */
abstract class SparkHoodieFileStatusCache {
  /**
...
private class SharedInMemoryCache(maxSizeInBytes: Long, cacheTTL: Long) extends Logging {
   ...
   */
  def createForNewClient(): SparkHoodieFileStatusCache = new SparkHoodieFileStatusCache {
 ...
 */
object NoopCache extends SparkHoodieFileStatusCache {
