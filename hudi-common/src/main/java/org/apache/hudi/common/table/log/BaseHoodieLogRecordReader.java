...
/**
 * New log record reader returning records of engine-specific data type
 */
public abstract class BaseHoodieLogRecordReader<T> {
  ...
  private void processDataBlock(HoodieDataBlock dataBlock, Option<KeySpec> keySpecOpt) throws Exception {
    ...
    try (ClosableIterator<T> recordIterator = recordsIteratorSchemaPair.getLeft()) {
      while (recordIterator.hasNext()) {
        ...
        // TODO(yihua): pass in meta info
        processNextRecord(nextRecord, Collections.emptyMap());
   ...
   */
  public abstract void processNextRecord(T hoodieRecord, Map<String, Object> newRecordInfo) throws Exception;
  ...
  private Pair<ClosableIterator<T>, Schema> getRecordsIterator(
    ...
    if (keySpecOpt.isPresent()) {
      ...
      blockRecordsIterator = dataBlock.getEngineRecordIterator(keySpec.getKeys(), keySpec.isFullKey());
    } else {
      blockRecordsIterator = dataBlock.getEngineRecordIterator();
    }

    // TODO(yihua): FIX schema on read (full schema evolution) here
    /*
